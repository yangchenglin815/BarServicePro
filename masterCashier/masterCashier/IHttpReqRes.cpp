...
void IHttpReqRes::slot_ReplyFinished()
{
	...
	{
		//AppConfigHandler.Info(QStringLiteral("ÍøÂçÒì³£: %1; errorCod:%2; errorMsg:%3").arg(m_sTestUrl).arg(err).arg(m_pReply->errorString()).toStdString());
...
{
	m_bOverTime = true;
	//AppConfigHandler.Info(QStringLiteral("ÍøÂç³¬Ê±£º%1").arg(m_sTestUrl).toStdString());
