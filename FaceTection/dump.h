...
LONG ApplicationCrashHandler(EXCEPTION_POINTERS *pException)
{
	...
	// 构建dump文件路径
	GetLocalTime(&stTime);
    QString path = QDir::toNativeSeparators(QDir::homePath() + "/AppData/Local/FaceTection/Dump");
	...
	// 这里弹出一个错误对话框并退出程序
    TSprintf(szMsg, (L"Sorry, FaceTection has crashed, please contact to us for help! \r\ndump file : %s"), szDumpFile);
